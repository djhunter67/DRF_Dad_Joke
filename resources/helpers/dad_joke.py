...
class Dad_Joke:
    ...
    def search_joke(self, search_term):
        ...
        self.search_data = get(
            f"{self.url}/search?term={search_term}", headers=self.header
        ).json()

        # Grab the jokes from the list of dictionaries
        try:
            search_results = self.search_data["results"][
                randint(1, self.search_data["total_jokes"] - 1)
            ]["joke"]
...
def main() -> None:
    ...
    search_data = input(f"{F.YELLOW}Please enter the search topic for a Dad Joke: {R}")
